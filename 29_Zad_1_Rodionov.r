...
x=5/0;x
#задаем значения первой колонки
n=c(1.2,2.3,4.5);n
#ззадаем значение второй колонки
s=c("aa","bb","cc");s
#Задаем значения третьей колонки
b=c(TRUE,FALSE,TRUE);b
#создаем data frame
df=data.frame(n,s,b)
df
#обращаемся к переменной n
df$n
#обращение к именам колонок
names(df)
#Обращение к именам строк
rownames(df)
#смотрим размерность таблицы (число строк и столбцов)
dim(df)
#смотрим общую статистическую характеристику
summary(df)

#считаем данные из файла csv в переменную pole
pole=read.table("pole123.csv", header=TRUE,sep=";",dec=".")
#смотрим что находится в переменной pole
pole

#посмотрим ранее созданный табличный объект df
df


#сохраним этот объект на диске в файле df1.csv
write.table(df,"df1.csv",sep=",",dec=".",quote=FALSE)

#сохраним этот же объект на диске в файле df2.csv
write.table(df,"df1.csv",sep=";",dec=",",quote=FALSE)
